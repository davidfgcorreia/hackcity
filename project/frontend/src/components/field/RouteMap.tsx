...
const numberIcon = (seq: number, done: boolean, active: boolean) =>
  L.divIcon({
    className: '',
    iconSize: [30, 30],
    iconAnchor: [15, 15],
    html: `<div style="width:30px;height:30px;border-radius:50%;display:flex;align-items:center;
      ...
      background:${done ? colors.grey : colors.danger};
...
export function RouteMap({ stops, position, cases, activeStopId, onSelect }: {
  ...
  return (
    <BaseMap>
      ...
      {stops.filter((s) => s.status !== 'removed').map((s) => (
        <Marker key={s.id} position={[s.lat, s.lng]}
          icon={s.kind === 'depot' ? depotIcon : numberIcon(s.seq, s.status === 'done', s.id === activeStopId)}
