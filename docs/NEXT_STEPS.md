...
- `/` is the operations home: eligible-bike alert, live mission start or continuation from a fresh GPS fix, links to Pickup, Case review and Insights, and a separate presentation simulation link.
...
- `/review` is the operational case map, list, evidence timeline and correction/approval workflow. `/insights` is the current read-only analysis of bicycle activity, recovery, station candidates, transit, journeys, weather and data quality. They remain separate in this version.
...
- The current Insights data is aggregated historical analysis. It cannot yet justify automatic dispatch or a final station installation decision. Candidate weights are provisional, and legal/safety feasibility has not been checked.
...
Rename **Insights** to **Data Analysis** in the home screen, navigation and page heading. Bring the case-review workflow into that workspace as an **Operations cases** view: map and list, evidence timeline, approval, corrections and audit trail. Keep `/review` as a redirect to the new view so existing links continue to work. Case decisions must continue to use the operational API and record the actor and reason; historical analytics stays read-only.

Build the workspace in this order:

1. **Unified data catalogue and filters.** Show every available source with its date range, update time, geographic coverage, row counts, missingness and meaning. Include bicycle trips/events and stations, parking and recovery, operational cases/missions, bus and rail stops and schedules, observed bus delays, card journey aggregates, weather and candidate areas. Keep 2025 and 2026 transit samples separate. Filter by date/time, operator, route, case status and geography where the underlying data permits; label unavailable combinations rather than implying a join.
2. **Multi-layer map.** Let users independently toggle bicycle stations and flows, out-of-station parking, recovery hotspots, candidate station areas, transit stops and frequency, observed delays, journey flows and live cases. Provide a clear legend, source and time window per layer. The 250 m grid is the base resolution; aggregate to 500 m, 1 km and municipality on zoom-out or by user choice. Aggregate counts from underlying cells, compute rates from summed numerators and denominators, and suppress small groups. Never average cell rates or expose individual card journeys.
3. **Drill-down and grouping.** Clicking a cell, station, stop or route opens its measures, time profile, source quality and related cases. Compare selected areas side by side; group by grid, parish/municipality, station catchment, operator/route and hour/day where supported. Keep map, table and chart selections synchronized. Export the selected aggregate view as CSV and a printable report with filters and definitions attached.
