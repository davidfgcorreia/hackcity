...
| `web` | 5173 | `/field`: mobile operator view; `/data`: planning map and live case review. |
...
| Re-sighted at the same spot with a newer `last_reported` | `located` | The clock continues; source observation time is saved as fresh evidence. |
| Same response repeated with unchanged `last_reported` | no location event | Poll time cannot make stale telemetry fresh. |
...
| Missing for longer than `GBFS_GONE_GRACE_S` (180 s) | `gbfs_disappeared`, `missing` | Last location is uncertain; case stays open for verification and any assigned stop is removed. The feed cannot distinguish trip, flat battery, or collection. |
...
- **Stored events:** transitions and genuinely new source location observations are stored in `vehicle_events` with `source=gbfs`. Folding them at startup restores parking clocks and freshness after an API restart. Cases expose the latest repeat observation separately from `updated_at`, which can advance during polling.
...
| `services/clock.py` | done | `now()` returns replay time in demo mode and wall-clock UTC otherwise; `mode()` says which (`replay` while a simulated clock is set). Missions, the home count and the field page use only the current mode's cases plus field reports (`cases.active_sources()`, `GET /cases?current=true`); switching modes re-plans active missions. |
...
| The 120 minutes count only 08:00–20:00 Europe/Lisbon (switchable) | `Rules.counted_minutes` via `core/enforcement.py`; the clock pauses outside the window; `ENFORCE_WINDOW=false` counts 24 h (`make enforce-window-on/off`); `/api/rules` exposes it | `test_clock_pauses_outside_08_to_20_lisbon`, `test_window_can_be_switched_off_*`, `test_enforcement.py` |
...
| `station_status_snapshots` | station_id, bikes_available, reported_at, collected_at | Prospective GBFS stock observations; stale reports are excluded from the balance benchmark. |
| `station_targets` | station_id, bikes, updated_at | Open hackathon edit for a station's occupancy benchmark. |
...
| GET | `/api/live/bikes`, `/api/stations/balance`, `/api/stations/{id}/balance-history` | done — source age is shown; no stock is inferred from bike trips |
| PUT | `/api/stations/{id}/target` `{bikes}` | done — open edit for the hackathon; add roles before operational use |
| GET | `/api/transit/vehicles`, `/api/transit/arrivals/{stop_id}` | done — TML GO and Carris Metropolitana, with freshness filtering |
